...
| — | 011_F3_DIAGNOSIS | PF-1-F3 [PF-2] | all 3 migrations applied; DB in sync; live PUT returns 204; historical log not available; F3 unresolvable deterministically — likely transient server state at time of walkthrough; no code defect found; production build stale (Apr26 12:39) — ops action needed to deploy Apr27 changes | ⚠ |
| — | 012_F3_DIAGNOSIS_V2 | PF-1-F3 [PF-2] v2 | Two root causes confirmed: (A) PM2 restart storm post-rebuild → server-down windows → nginx 503 (~16/17 failures); (B) PrismaClientValidationError round 12 hole 18 gross:undefined → app 500 (1/17); live PUT now returns 204, zero new errors, server stable 47m; no application code defect; ops fix: pm2 stop then pm2 start instead of pm2 restart during rebuilds | ✓ |
