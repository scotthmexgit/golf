...
- [ ] **Round-state verification agent (post-#8)**: propose a new agent (or extension of reviewer) that runs against a `ScoringEventLog` + `RoundConfig` and verifies per-hole + per-round invariants — zero-sum per bet, zero-sum across overlapping-participant bets, junk award player validity, `MatchState` consistency with event history, integer invariants, hole coverage. Run shape is batch/end-of-round, not per-turn. Prerequisite: `aggregate.ts` stable (all of #8 closed). `ZeroSumViolationError` in `aggregate.ts` is a partial runtime analogue. Added 2026-04-24.
