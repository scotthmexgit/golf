...
interface ScoreRowProps {
  ...
  // When false, bet row shows "—" instead of the computed delta (SKINS-2 suppression).
  showBetDelta?: boolean
  // Called when user changes the stepper value — signals an intentional score edit.
  onScoreEdit?: () => void
  // Callback to open the BetDetailsSheet for full per-round per-game breakdown.
  onOpenSheet?: () => void
}

export default function ScoreRow({
  player, hole, par, holeIndex, score, dots,
  activeGames, showJunkDots, holeTotal, showBetDelta = true, onScoreEdit, onOpenSheet,
}: ScoreRowProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Stepper
            value={score || par}
            initialValue={par}
            onChange={(v) => { setScore(player.id, hole, v); onScoreEdit?.() }}
            min={1}
            max={15}
          />
          {diff !== null && (
      ...
      {holeDelta !== null && (
        <>
          {/* Bet row — tap to open BetDetailsSheet for full round breakdown.
          ...
          >
            ...
            <span
              ...
              style={{
                color: showBetDelta && holeDelta > 0 ? '#22c55e' : showBetDelta && holeDelta < 0 ? 'var(--red-card)' : 'var(--muted)',
              }}
            >
              {showBetDelta ? formatMoneyDecimal(holeDelta) : '—'}
            </span>
          </button>
        </>
      )}
