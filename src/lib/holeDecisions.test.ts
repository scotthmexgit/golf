...
describe('buildHoleDecisions — returns null when nothing to persist', () => {
  ...
  it('presses excluded when nassau not in gameTypes', () => {
    expect(buildHoleDecisions(makeHole({ presses: ['front'] }), WOLF_SET)).toBeNull()
  })
  it('empty presses array returns null', () => {
...
describe('buildHoleDecisions — includes set fields', () => {
  ...
  it('presses included when nassau active', () => {
    const blob = buildHoleDecisions(makeHole({ presses: ['front', 'back'] }), NASSAU_SET)
    expect(blob).toEqual({ presses: ['front', 'back'] })
  })
  it('wolfPick + presses when both game types active', () => {
    const blob = buildHoleDecisions(makeHole({ wolfPick: 'solo', presses: ['front'] }), BOTH_SET)
    expect(blob).toEqual({ wolfPick: 'solo', presses: ['front'] })
  })
...
describe('validateHoleDecisions — accepts valid inputs', () => {
  ...
  it('presses string[] passes', () => {
    expect(validateHoleDecisions(NASSAU_SET, { presses: ['front', 'press-1'] }).ok).toBe(true)
  })
...
describe('validateHoleDecisions — per-type gating (active game types)', () => {
  ...
  it('presses accepted when nassau is active', () => {
    expect(validateHoleDecisions(NASSAU_SET, { presses: ['front'] }).ok).toBe(true)
  })
  it('empty gameTypes set skips per-type gating (structural check only — used at hydration)', () => {
    ...
    expect(validateHoleDecisions(EMPTY_SET, { presses: ['front'] }).ok).toBe(true)
  })
})
...
describe('validateHoleDecisions — rejects unknown keys (rule #7)', () => {
  ...
  it('presses with non-string element rejected', () => {
    const r = validateHoleDecisions(NASSAU_SET, { presses: ['front', 42] })
    expect(r.ok).toBe(false)
  })
})
...
describe('hydrateHoleDecisions — round-trip fidelity', () => {
  ...
  it('presses survive round-trip', () => {
    const blob = buildHoleDecisions(makeHole({ presses: ['front'] }), NASSAU_SET)
    const result = hydrateHoleDecisions(blob)
    expect(result.presses).toEqual(['front'])
  })
...
describe('Full round-trip: build → validate → hydrate', () => {
  ...
  it('Nassau: press confirmation on hole 5 survives', () => {
    const hole = makeHole({ number: 5, presses: ['front'] })
    const blob = buildHoleDecisions(hole, NASSAU_SET)!
    expect(validateHoleDecisions(NASSAU_SET, blob).ok).toBe(true)
    expect(hydrateHoleDecisions(blob).presses).toEqual(['front'])
  })
  it('Multiple decisions survive together', () => {
    const hole = makeHole({ wolfPick: 'solo', presses: ['front'], bangoWinner: 'p3' })
